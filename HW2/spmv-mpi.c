...
double benchmark_coo_spmv(coo_matrix * coo, float* x, float* y)
{
    ...
    float *y1 = (float *)calloc(num_rows, sizeof(float));
    float *y2 = (float *)calloc(num_rows, sizeof(float));
    float *y3 = (float *)calloc(num_rows, sizeof(float));
    float *y4 = (float *)calloc(num_rows, sizeof(float));
    float *y5 = (float *)calloc(num_rows, sizeof(float));
    float *y6 = (float *)calloc(num_rows, sizeof(float));
    float *y7 = (float *)calloc(num_rows, sizeof(float));

    if (process_number != 0)
    {
        MPI_Send(y, num_rows, MPI_FLOAT, 0, 100, MPI_COMM_WORLD); // Send y[] to process-0
    }

    else
    {
        MPI_Recv(y1, num_rows, MPI_FLOAT, 1, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y2, num_rows, MPI_FLOAT, 2, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y3, num_rows, MPI_FLOAT, 3, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y4, num_rows, MPI_FLOAT, 4, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y5, num_rows, MPI_FLOAT, 5, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y6, num_rows, MPI_FLOAT, 6, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y7, num_rows, MPI_FLOAT, 7, 100, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        //Accumulate values from the all the received arrays
        for (int i=0; i<num_rows; i++)
        {
            y[i] = y[i] + y1[i] + y2[i] + y3[i] + y4[i] + y5[i] + y6[i] + y7[i]; 
        }
